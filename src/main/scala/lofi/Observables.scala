...
object Observable {
  ...
  def toTex[A](input: Observable[A])(time: String): String = input match {
    ...
    case ReduceR(o, origin, z, Add) => val s = "s"; s"\\sum_{s=$origin}^{s=$time}{${toTex(o)(s)}}"
    case ReduceR(o, origin, z, Mul) => val s = "s"; s"\\prod_{s=$origin}^{s=$time}{${toTex(o)(s)}}"
    case ReduceR(o, origin, z, Max) => val s = "s"; s"\\max_{s=$origin}^{s=$time}{${toTex(o)(s)}}"
    case ReduceR(o, origin, z, Avg) => val s = "s"; s"\\int_{s=$origin}^{s=$time}{${toTex(o)(s)}}"
